...
void DrawProbEllipse(VectorXd x, const MatrixXd& C,
    ...
    float a = sqrt(0.5*(sxx+syy+sqrt(pow(sxx-syy, 2)+
        4*pow(sxy, 2))));   // always greater
    float b = sqrt(0.5*(sxx+syy-sqrt(pow(sxx-syy, 2)+
        4*pow(sxy, 2))));   // always smaller
...
void Plot_State(const VectorXd& mu, const MatrixXd& sigma,
        const Mapper& mapper, const vector<bool>&observedLandmarks,
        const vector<LaserReading>& Z) {
    // initial robot pose and landmarks
